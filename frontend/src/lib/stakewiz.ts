...
export async function getStakeWizValidators() {
    const response = await fetch('https://api.stakewiz.com/validators?sort=-stake_weight');
    if (!response.ok) {
        throw new Error(`StakeWiz validators request failed (${response.status})`);
    }
