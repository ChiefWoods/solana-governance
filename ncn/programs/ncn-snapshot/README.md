...
| `init_ballot_box`             | Governance Program PDA                | Initializes a new `BallotBox` for a snapshot selection vote. Requires `snapshot_slot` parameter which must be greater than current slot. Snapshot of `whitelisted_operators` is stored in `voter_list`. |
